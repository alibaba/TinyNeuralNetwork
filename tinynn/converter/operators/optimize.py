...
class GraphOptimizer(object):
    ...
    def fuse_bmm_add_pass(self):
        ...
        for (bmm, add), mapping in zip(sorted_ops, restore_mapping):
            ...
            output0 = self.create_transform_tensor(np.matmul(new_input_tensor0.tensor, weight_transformed0.tensor), quantization=output_tensor.quantization)
            ...
            output1 = self.create_transform_tensor(np.matmul(new_input_tensor1.tensor, weight_transformed1.tensor), quantization=output_tensor.quantization)
            ...
            output2 = self.create_transform_tensor(np.matmul(new_input_tensor2.tensor, weight_transformed2.tensor), quantization=output_tensor.quantization)
            ...
            pre_output = self.create_transform_tensor(np.concatenate((out_put_tensor0, out_put_tensor1, out_put_tensor2), axis=0), quantization=output_tensor.quantization)
            ...
            ops.append(tfl.ReshapeOperator([pre_output, shape_output_tensor], [output_tensor], output_shape))
