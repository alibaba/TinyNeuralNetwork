...
class GraphOptimizer(object):
    ...
    def bmm_rewrite_pass(self):
        ...
        for bmm, mapping in zip(sorted_ops, restore_mapping):
            ...
            new_weight_transformed = self.create_transform_tensor(np.transpose(new_weight_tensor.tensor, perm), quantization=new_weight_tensor.quantization)
            ops.append(tfl.TransposeOperator([new_weight_tensor, perm_tensor], [new_weight_transformed]))
            
            
            input_dim = 0
            input_chunks = new_input_tensor.shape[input_dim]

            new_inputs = np.array(new_input_tensor.tensor)
            new_input0 = new_inputs[0, :, :]
            new_input_tensor0 = self.create_transform_tensor(np.array(new_input0), quantization=new_input_tensor.quantization)
            new_input1 = new_inputs[1, :, :]
            new_input_tensor1 = self.create_transform_tensor(np.array(new_input1), quantization=new_input_tensor.quantization)
            new_input2 = new_inputs[2, :, :]
            new_input_tensor2 = self.create_transform_tensor(np.array(new_input2), quantization=new_input_tensor.quantization)
            
            new_input_tensors = [new_input_tensor0, new_input_tensor1, new_input_tensor2]
            ops.append(tfl.UnpackOperator([new_input_tensor], new_input_tensors, input_chunks, input_dim))

            weight_dim =0
            weight_chunks = new_weight_transformed.shape[weight_dim]

            new_weights = np.array(new_weight_transformed.tensor)
            new_weight0 = new_weights[0, :, :]
            new_weight_tensor0 = self.create_transform_tensor(np.array(new_weight0), quantization=new_weight_transformed.quantization)
            new_weight1 = new_weights[1, :, :]
            new_weight_tensor1 = self.create_transform_tensor(np.array(new_weight1), quantization=new_weight_transformed.quantization)
            new_weight2 = new_weights[2, :, :]
            new_weight_tensor2 = self.create_transform_tensor(np.array(new_weight2), quantization=new_weight_transformed.quantization)


            new_weight_tensors = [new_weight_tensor0, new_weight_tensor1, new_weight_tensor2]
            ops.append(tfl.UnpackOperator([new_weight_transformed], new_weight_tensors, weight_chunks, weight_dim))

            if str(new_input_tensor.dtype) == 'float32':
               bias_type = 'float32'
            else:
               bias_type = 'int32'
            ...
            # shape_weight_tensor0 = self.create_attr_tensor(np.array(weight_shape0, dtype=bias_type))
            ...
            # perm = [0, 2, 1]
            # perm_tensor0 = self.create_attr_tensor(np.array(perm, dtype='int32'))
            # weight_transformed0 = self.create_transform_tensor(np.transpose(new_weight_tensor0.tensor, perm), quantization=new_weight_tensor0.quantization)
            # ops.append(tfl.TransposeOperator([new_weight_tensor0, perm_tensor0], [weight_transformed0]))
            
            # new_weight_transformed_shape0 = weight_transformed0.shape[1:]
            # shape_new_weight_transformed0 = self.create_attr_tensor(np.array(new_weight_transformed_shape0, dtype='int32'))
            ...
            bias_tensor0 = self.create_attr_tensor(np.zeros(new_weight_tensor0.shape[1], dtype=bias_type))

            new_input_tensor0, new_weight_tensor0

            perm0 = [1, 0]
            perm_tensor0 = self.create_attr_tensor(np.array(perm, dtype='int32'))
            weight_transformed0 = self.create_transform_tensor(np.transpose(new_weight_tensor0.tensor, perm0), quantization=new_weight_tensor0.quantization)
            ops.append(tfl.TransposeOperator([new_weight_tensor0, perm_tensor0], [weight_transformed0]))


            input0 = [new_input_tensor0, weight_transformed0, bias_tensor0]
            output0 = self.create_transform_tensor(np.matmul(new_input_tensor0.tensor, weight_transformed0.tensor), quantization=output.quantization)
            ...
            shape_output0 = self.create_attr_tensor(np.array(output_shape0, dtype='int32'))
            ...
            # weight_transformed1 = self.create_transform_tensor(np.transpose(new_weight_tensor1.tensor, perm), quantization=new_weight_tensor1.quantization)
            # ops.append(tfl.TransposeOperator([new_weight_tensor1, perm_tensor1], [weight_transformed1]))
            ...
            bias_tensor1 = self.create_attr_tensor(np.zeros(new_weight_tensor1.shape[1], dtype=bias_type))

            new_input_tensor1, new_weight_tensor1

            perm1 = [1, 0]
            perm_tensor1 = self.create_attr_tensor(np.array(perm, dtype='int32'))
            weight_transformed1 = self.create_transform_tensor(np.transpose(new_weight_tensor1.tensor, perm1), quantization=new_weight_tensor1.quantization)
            ops.append(tfl.TransposeOperator([new_weight_tensor1, perm_tensor1], [weight_transformed1]))


            input1 = [new_input_tensor1, weight_transformed1, bias_tensor1]
            output1 = self.create_transform_tensor(np.matmul(new_input_tensor1.tensor, weight_transformed1.tensor), quantization=output.quantization)
            ...
            # weight_transformed2 = self.create_transform_tensor(np.transpose(new_weight_tensor2.tensor, perm), quantization=new_weight_tensor2.quantization)
            # ops.append(tfl.TransposeOperator([new_weight_tensor2, perm_tensor2], [weight_transformed2]))
            ...
            bias_tensor2 = self.create_attr_tensor(np.zeros(new_weight_tensor2.shape[1], dtype=bias_type))

            new_input_tensor2, new_weight_tensor2

            perm2 = [1, 0]
            perm_tensor2 = self.create_attr_tensor(np.array(perm, dtype='int32'))
            weight_transformed2 = self.create_transform_tensor(np.transpose(new_weight_tensor2.tensor, perm2), quantization=new_weight_tensor2.quantization)
            ops.append(tfl.TransposeOperator([new_weight_tensor2, perm_tensor2], [weight_transformed2]))


            input2 = [new_input_tensor2, weight_transformed2, bias_tensor2]
            output2 = self.create_transform_tensor(np.matmul(new_input_tensor2.tensor, weight_transformed2.tensor), quantization=output.quantization)
            ...
            pre_output = self.create_transform_tensor(np.concatenate((out_put_tensor0, out_put_tensor1, out_put_tensor2), axis=0), quantization=output.quantization)
            ...
            ops.append(tfl.ReshapeOperator([pre_output, shape_output_tensor], [output], output_shape))
            for op in ops:
                self.graph.add_operator(op, transform=True)
...
def is_bmm_node(vertex: ig.Vertex, graph_converter: ig.Graph, bmm_rewrite: bool):
    return (vertex['node_type'] == ExtendedOperator.BATCH_MATMUL)
