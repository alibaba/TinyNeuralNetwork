...
from tinynn.util.bn_restore import model_restore_bn, ConvBnTrain, get_submodule_with_parent_from_name
...
def bn_scale(conv, scale):
    if hasattr(conv, 'fused_bn'):
        conv.fused_bn.weight.data = conv.fused_bn.weight.data * scale
        conv.fused_bn.bias.data = conv.fused_bn.bias.data * scale
...
def _cross_layer_equalize(model: nn.Module, dummy_input, threshold=1000) -> Tuple[list, nn.Module]:
    ...
    """
    with torch.no_grad() and model_tracer():
...
def cross_layer_equalize(
    model: nn.Module, dummy_input, device, threshold=1000, work_dir="out", cle_iters=2
) -> nn.Module:
    ...
    log.info("start to do High Bias Absorb.")
...
def bias_absorb_helper_(layer1, layer2, model, origin_model):
    ...
    if isinstance(pre_layer, ConvBnTrain) and isinstance(cur_layer, ConvBnTrain):
        ...
        if hasattr(pre_layer, 'fused_bn') and hasattr(cur_layer, 'fused_bn'):
            pre_bn = pre_layer.fused_bn
            cur_conv = cur_layer
        else:
            # High Bias Absorbing is not supported for models without BatchNorm Layers
            return
...
def __high_bias_absorb(
    ...
    """The debug API which use to do bn_restore after CLE, then do HBA."""
    cle_model.to(device)
    ...
    if use_origin_bn:
        ...
        if cali_func is None:
            log.warning(
                "High Bias Fold can not running, you can setting args as below:\n"
                "1. If your origin model has bn, please set `bn_fuse=True` at `cross_layer_equalize` "
                "and set `use_origin_bn=True` at "
                "`high_bias_absorb`\n"
...
def high_bias_absorb(cle_model, device, layer_groups):
    """Fold bias value greater than 3 * sigma to next layer's bias.
...
def model_fuse_bn(model: nn.Module, dummy_input):
    """fuse bn to conv inplace, and attach the origin bn to fused conv with attr:`fused_bn`"""
    with model_tracer() and torch.no_grad():
        model.eval()
        ...
        for quant_nodes in quant_list:
            ...
            setattr(getattr(graph.module, quant_nodes[0]), 'fused_bn', bn_cur)
...
def clear_model_fused_bn(model: nn.Module):
    ...
    for mod in model.modules():
        if isinstance(mod, nn.Conv2d) and hasattr(mod, 'fused_bn'):
            delattr(mod, 'fused_bn')


def relu6_2_relu(model):
    """replace relu6 to relu inplace"""
    for name, mod in model.named_modules():
        if isinstance(mod, nn.ReLU6):
            cur_mod, parent_mod, name = get_submodule_with_parent_from_name(model, name)
            setattr(parent_mod, name, torch.nn.ReLU(inplace=cur_mod.inplce))
